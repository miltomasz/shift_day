...
module ShiftDay
  class TestShiftDay < MiniTest::Unit::TestCase
    ...
    def test_calculating_shifted_date
      assert_equal(@shifted_date, Base.new(@date_before_shifted, 6).shifted_date)
      assert_equal(@shifted_date, Base.new(@date_after_shifted, 6).shifted_date)
    end

    def test_calculating_date_range_when_date_before
      date_range = @shifted_date_minus_24..@shifted_date
      assert_equal(date_range, Base.new(@date_before_shifted, 6).date_range)
    end
